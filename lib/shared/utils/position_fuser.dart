import 'package:easylocate_flutter_sdk/utils/geotools.dart';
import 'package:latlong2/latlong.dart';
import 'package:vector_math/vector_math.dart';

import 'position_transformation.dart';

/// A position fuser that uses kalman filter to combine incoming positions from gnss and uwb sources.
class PositionFuser {
  final Wgs84Reference _wgs84reference;

  PositionFuser({
    required double referenceLatitude,
    required double referenceLongitude,
    required double referenceAzimuth,
  }) : _wgs84reference = Wgs84Reference(
            referenceLatitude, referenceLongitude, referenceAzimuth);

  Vector2? _position;

  Matrix2? _covariance;

  /// The time when the PositionFuser was last updated
  DateTime? _timeOfLastUpdate;

  bool _isInitialised = false;

  /// Returns true if the PositionFuser is already initialized
  bool get isInitialised => _isInitialised;

  /// Updates a uwbPosition received
  void updateUwbPosition(LatLng? position, double accuracy) {
    _timeUpdate(DateTime.now());
    if (position != null && accuracy > 0 && accuracy < 3) {
      _measurementUpdate(position, accuracy);
    }
  }

  /// Updates a gnssPosition received
  void updateGnssPosition(LatLng? position, double accuracy) {
    _timeUpdate(DateTime.now());
    if (position != null &&
        (position.latitude != 0 && position.longitude != 0) &&
        accuracy < 15) {
      _measurementUpdate(position, accuracy);
    }
  }

  LatLng? get fusedPosition {
    if (_isInitialised) {
      _timeUpdate(DateTime.now());
      if (_position != null && _covariance != null) {
        return _wgs84reference.convertToLatLng(_position!);
      }
    }
    return null;
  }

  /// Updates the time, and the covariance based on the uncertainty in the system model
  void _timeUpdate(DateTime time) {
    if (_timeOfLastUpdate != null) {
      final timeDiff = time.difference(_timeOfLastUpdate!);
      if (_isInitialised) {
        //predicted change in position for a person walking is considered to be around 1 meter/second,
        // but for a moving vehicle it can be more. For now we've considered 25km/h (around 6.9 m/s)
        const positionVariance = 25.0 / 3.6;
        // Generally Represented by Q in the Kalman filter, this can be considered as the amount of uncertainty in the system model
        final Matrix2 systemUncertainty = Matrix2.identity() *
            (timeDiff.inMilliseconds / 1000.0 * positionVariance);
        // Calculates the new covariance when based on the previous error and the uncertainty in the system model
        _covariance = _covariance! + systemUncertainty;
      }
    }
    _timeOfLastUpdate = time;
  }

  /// Updates the position and covariance based on Kalman filter which requires the new measurement and covariance.
  void _measurementUpdate(LatLng measuredPosition, double measuredAccuracy) {
    // The kalman filter predictions are based currently on values in m/s and therefore it is necessary to covert LatLng values back to meters
    final position = _wgs84reference.convertToVector2(measuredPosition);
    final covariance =
        Matrix2.identity() * (measuredAccuracy * measuredAccuracy);
    if (_isInitialised) {
      // The predicted uncertainty takes into account the previous error estimate and the current error from the measurement
      final predictedUncertainty = _covariance! + covariance;
      predictedUncertainty.invert();
      // The kalman gain takes into account the error in the measurement and the error the predicted value
      final kalmanGain = _covariance! * predictedUncertainty;
      // The residual is the difference between the new position(measured) and the value predicted by the filter
      final residual = position - _position!;
      // Calculates the new position and covariance based on the kalman gain and the residual
      _position = _position! + (kalmanGain * residual);
      _covariance = (Matrix2.identity() - kalmanGain) * _covariance!;
    } else {
      if (_timeOfLastUpdate != null) {
        _position = position;
        _covariance = covariance;
        _isInitialised = true;
      }
    }
  }
}
